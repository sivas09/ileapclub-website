...
export function buildAgendaRtf(meeting: AgendaMeeting) {
  ...
  return [
    ...
    paragraph(`SESSION: ${meeting.title}`),
    paragraph(`CLUB: ${meeting.club.name}`),
    paragraph(`CENTRE: ${meeting.club.centre.name}`),
    meeting.startTime ? paragraph(`TIME: ${meeting.startTime}`) : "",
    meeting.location ? paragraph(`LOCATION / ONLINE LINK: ${meeting.location}`) : "",
    "\\par",
    tableHeader(["Time", "Agenda Item", "Roles / Members"]),
    agendaRows,
    "\\par",
    tableHeader(["Role", "Assigned Member"]),
    assignmentRows || tableRow(["No roles configured", "None"]),
    "\\par",
...
function tableRow(values: string[], isHeader = false) {
  ...
  const cellWidths = values.length <= 2
    ? ["\\cellx4200", "\\cellx10000"]
    : ["\\cellx1600", "\\cellx4700", "\\cellx10000"];
